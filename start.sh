...
echo "create database magento" | mysql -u "$DB_USER" --password="$DB_PASS" -h db -P "$DB_PORT_3306_TCP_PORT"

while [ $? -ne 0 ]; do
	sleep 5
        echo "create database magento" | mysql -u "$DB_USER" --password="$DB_PASS" -h db -P "$DB_PORT_3306_TCP_PORT"
        echo "show tables" | mysql -u "$DB_USER" --password="$DB_PASS" -h db -P "$DB_PORT_3306_TCP_PORT" magento
done
